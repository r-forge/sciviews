...
}, run the code that should generate a warning inside \code{\inputencoding{latin1}checkException()\selectlanguage{english}
}, and then restore default warning behavior with \code{\inputencoding{latin1}options(warn = owarn)\selectlanguage{english}
}.%
...
trap wrong arguments, for instance, like in \code{\inputencoding{latin1}checkException(Square(\textquotedbl{}xx\textquotedbl{}))\selectlanguage{english}
} here above (a character string is provided where a numerical value
is expected).

Now, let's look what happens if we test the \code{\inputencoding{latin1}Cube()\selectlanguage{english}
...
wrong for the sake of the demonstration) from \code{Cube(\inputencoding{latin1}{2}\inputencoding{latin9}:3)}
in \code{checkEqualsNumeric(c(8, 28), Cube(\inputencoding{latin1}{2}\inputencoding{latin9}:3))},
and (of course), we got \code{c(8, 27)}. This (misspecified) test
shows how \pkg{svUnit} presents test failures.
\item The results of the tests on \code{\inputencoding{latin1}Cube()\selectlanguage{english}
} are added to the previous report. So, it is possible to build rather
easily reports that summarize tests on several objects, by adding
test results in the logger sequentially. \pkg{svUnit} does this naturally
and transparently. Starting a new report is equally simple: just use
\code{\inputencoding{latin1}clearLog()\selectlanguage{english}
}...
\end{enumerate}
...
by a \code{\inputencoding{latin1}checkXXX()\selectlanguage{english}
} function in \pkg{svUnit}/\pkg{RUnit}. Five such functions are currently
defined:
\begin{lyxlist}{00.00.0000}
\item [{\code{\inputencoding{latin1}checkEquals(current, target)\selectlanguage{english}
}}] determines if data in \code{\inputencoding{latin1}target\selectlanguage{english}
} is the same as data in \code{\inputencoding{latin1}current\selectlanguage{english}
}.
\item [{\code{\inputencoding{latin1}checkEqualsNumeric(current, target)\selectlanguage{english}
}}] does the same but allows for a better comparison for numbers (variation
allowed within a tolerance window).
\item [{\code{\inputencoding{latin1}checkIdentical(current, target)\selectlanguage{english}
}}] checks whether two \R{} objects are strictly identical.
\item [{\code{\inputencoding{latin1}checkTrue(expr)\selectlanguage{english}
}}] only succeed if \code{\inputencoding{latin1}expr\selectlanguage{english}
} is \code{\inputencoding{latin1}TRUE\selectlanguage{english}
}. Note a difference in \pkg{svUnit} and \pkg{RUnit} (at least, in
its version 0.4-17): the \pkg{RUnit} function is not vectorized and
\code{\inputencoding{latin1}expr\selectlanguage{english}
...
elements of the vector must be \code{\inputencoding{latin1}TRUE\selectlanguage{english}
} for the test to succeed. When you make sure that \code{\inputencoding{latin1}expr\selectlanguage{english}
} always returns a single logical value (for instance by using \code{\inputencoding{latin1}all(expr)\selectlanguage{english}
}), both functions should be compatible.
\item [{\code{\inputencoding{latin1}checkException(expr)\selectlanguage{english}
}}] verifies that a given code raises an exception (in \R{}, it means
that a line of code with \code{\inputencoding{latin1}stop()\selectlanguage{english}
} is executed).
\item [{DEACTIVATED()}] makes sure that all tests following this instruction
(in a test function, see next paragraph) are deactivated, and inserts
a reminder in the logger about the fact that some tests are deactivated
in this suite.
\end{lyxlist}
For all these functions, you have an additional optional argument
\code{\inputencoding{latin1}msg\selectlanguage{english}
} where you can provide a (short) message to print in front of each
text in the report. These functions return invisibly: \code{\inputencoding{latin1}TRUE\selectlanguage{english}
} if the test succeeds, or \code{\inputencoding{latin1}FALSE\selectlanguage{english}
} if it fails (code is executed correctly, but does not pass the test),
and \code{\inputencoding{latin1}NA\selectlanguage{english}
...
of the test and the timing in a logger (object \var{svSuiteData}
inheriting from \var{environment}) called \var{.Log} and located
in the user\textquoteright{}s workspace. So, executing a series of
assertions and getting a report is simply done as (in its simplest
form, you can use the various \code{checkXXX()} functions directly
at the command line):
...
As you can see, the \code{\inputencoding{latin1}checkXXX()\selectlanguage{english}
} functions work hand in hand with the test logger (the \code{\inputencoding{latin1}checkXXX()\selectlanguage{english}
} functions also return the result of the test invisibly, so, you can
also assign it to a variable if you like). These function are mainly
used for their side-effect of adding an entry to the logger.

The last command \code{\inputencoding{latin1}Log()\selectlanguage{english}
} prints the content of the logger. You see how a report is printed,
with a first part being a short summary by categories (assertions
run at the command line are placed automatically in the \var{eval}
category: there is no better context known for them. Usually, those
assertions should be placed in test functions, or in test units, as
we will see later in this manual, and the category will reflect this
organization). A detailed report on the tests that failed or raised
an error is also printed at the end of the report. 

Of course, the same report is much easier to manipulate from within
the graphical tree in the Komodo\textquoteright{}s \textbf{R Unit}
tab, but the simple text report in \R{} has the advantage of being
...
clickable list of all tests run. From this point, it is relatively
easy to define nightly cron task jobs on a server to run a script
that executes these tests and update a wiki page (look at your particular
wiki engine documentation to determine how you can access wiki pages
on the command line).
...
the logger from the command line, in particular, \code{\inputencoding{latin1}stats()\selectlanguage{english}
}, \code{\inputencoding{latin1}summary()\selectlanguage{english}
}, \code{\inputencoding{latin1}metadata()\selectlanguage{english}
} and \code{\inputencoding{latin1}ls()\selectlanguage{english}
}:
...
As you can see, \code{\inputencoding{latin1}ls()\selectlanguage{english}
} lists all components recorded in the test suite. Each component is
a \var{svTestData} object inheriting from \var{data.frame}, and
it can be easily accessed through the \code{\inputencoding{latin1}\$\selectlanguage{english}
} operator. There are, of course similar methods defined for those
\var{svTestData} objects, like \code{\inputencoding{latin1}print()\selectlanguage{english}
}, \code{\inputencoding{latin1}summary()\selectlanguage{english}
} and \code{\inputencoding{latin1}stats()\selectlanguage{english}
}:
...
whose name must start with \code{\inputencoding{latin1}test\selectlanguage{english}
}. It typically contains a series of assertions applied to one object,
method, or function to be checked (this is not obligatory, assertions
are not restricted to one object, but good practices strongly suggest
such a restriction). Here is an example:
...
using \code{\inputencoding{latin1}runTest()\selectlanguage{english}
...
is simply made using \code{\inputencoding{latin1}test(myobj) <-\selectlanguage{english}
}:
...
in particular, \command{\inputencoding{latin1}R CMD check\selectlanguage{english}
}. In both \pkg{RUnit}, and \pkg{svUnit}, one can define such test
units, and they are made code compatible between the two implementations
(at least for version 0.4-17 of \pkg{RUnit}).

A test unit is a sourceable text file that contains one or more test
functions, plus possibly \code{\inputencoding{latin1}.setUp()\selectlanguage{english}
} and \code{\inputencoding{latin1}.tearDown()\selectlanguage{english}
...
as separate \var{svTest} objects) by using \code{\inputencoding{latin1}makeUnit()\selectlanguage{english}
}. Here is how you promote the test associated with our \code{\inputencoding{latin1}Square()\selectlanguage{english}
...
with the \code{\inputencoding{latin1}dir =\selectlanguage{english}
} argument of \code{\inputencoding{latin1}makeUnit()\selectlanguage{english}
} for a more permanent record of this test unit file. Note also that
\code{\inputencoding{latin1}.setUp()\selectlanguage{english}
} and \code{\inputencoding{latin1}.tearDown()\selectlanguage{english}
} functions are constructed automatically for you. They specify the
context of these tests. This context is used, for instance, by the
GUI in Komodo to locate the test function and the code being tested.
\begin{lyxcode}
\#\#~Test~unit~'Square'
...
is embedded in a \code{\inputencoding{latin1}if ({}``package:svUnit'' \%in\% search())\selectlanguage{english}
...
test units with the same \code{\inputencoding{latin1}makeUnit()\selectlanguage{english}
} function. But for the moment, let\textquoteright{}s discuss a little
bit how such test units should be organized in \R{} package.
...
\command{\inputencoding{latin1}R CMD check\selectlanguage{english}
} process of your packages. You do thit by means of examples in a help
page (we prefer to use \textbf{examples}, instead of \file{/tests}
in the \command{\inputencoding{latin1}R CMD check\selectlanguage{english}
} process, because examples offer a more flexible way to run tests
and you can also run them in interactive sessions through the \code{\inputencoding{latin1}example()\selectlanguage{english}
} \R{} function, which is not the case for code located in the \file{/tests}
subdirectory of your package). Here is what you do to associate some
or all of your unit tests to the \command{\inputencoding{latin1}R CMD check\selectlanguage{english}
} process (illustrated with the case of the \pkg{svUnit} package itself):
\begin{itemize}
...
\item The important part of this file is, of course, the \code{\inputencoding{latin1}\textbackslash{}examples\{\}\selectlanguage{english}
} section. You must first clear the log, then run each test, and then,
call the \code{\inputencoding{latin1}errorLog()\selectlanguage{english}
} function. That function looks if one or more tests failed or raised
an error. In this case, it stops execution of the example and causes
a dump of the test log in the \command{\inputencoding{latin1}R CMD check\selectlanguage{english}
} process. That way, providing that you have the \pkg{svUnit} package
installed in the machine where you run \command{\inputencoding{latin1}R CMD check\selectlanguage{english}
}, your test units will be included nicely in the checking process
of your packages, that is, they will run silently each time you check
...
In the present case, the \code{\inputencoding{latin1}errorLog()\selectlanguage{english}
} instruction in the examples returns nothing, because all tests succeed.
If there is an error somewhere, you will see it printed at the end
of this example.
...
in a very convenient way: just specify \code{\inputencoding{latin1}package:myPkg\selectlanguage{english}
} and all test units in the \file{/unitTests} subdirectory of the
package \pkg{myPkg} will be included (\pkg{svUnit} does all the
required work to map these to actual directories where the test unit
files are located). Also, if you specify \code{\inputencoding{latin1}package:myPkg (subgroup)\selectlanguage{english}
}, you will include the test units defined in \file{/unitTests/subgroup}
in the package \pkg{myPkg}. Of course, you will be able to add also
test units defined in custom directories, outside of \R{} packages
(for instance for integration of \emph{harness tests} that check cross-packages,
or multi-packages features of your application).

Test functions associated to your test suite receive a special treatment.
Unlike \code{\inputencoding{latin1}runTest()\selectlanguage{english}
} applied to a single test function, or to an object that has an associated
test function, these tests are not run from the version loaded in
memory. Instead, they are first collected together in a test unit
file on disk (located in the \R{} temporary directory, by default),
and run from there. Hence, building a more complex test unit file
by collecting together several test functions is just a question of
constructing a test suite, and then, applying the \code{\inputencoding{latin1}makeUnit()\selectlanguage{english}
} function to this \var{svSuite} object.

Before we apply all this, you should also know one more function:
\code{\inputencoding{latin1}svSuiteList()\selectlanguage{english}
}. This function lists all test units and test functions available
in your system at a given time. So, you don\textquoteright{}t need
to manually create lists of components. You are better to list them
automatically. Of course, this function has a lot of arguments for
listing only test units in packages, only test functions, specifying
...
One argument is particularly important: \code{\inputencoding{latin1}excludeList =\selectlanguage{english}
...
you will certainly not want to run again and again, let\textquoteright{}s
...
always be listed by \code{\inputencoding{latin1}svSuiteList()\selectlanguage{english}
},... unless you define an adequate filter expression that will exclude
them from your list)! As the default argument suggests it, the regular
expression for list exclusion could also be recorded in \code{\inputencoding{latin1}options(svUnit.excludeList = ...)\selectlanguage{english}
...
Thus, every entry matching the regular expressions \code{\inputencoding{latin1}package:sv\selectlanguage{english}
} and \code{\inputencoding{latin1}package:RUnit\selectlanguage{english}
} are excluded from the list. The entries \code{\inputencoding{latin1}package:svUnit\selectlanguage{english}
} and \code{\inputencoding{latin1}package:svUnit (VirtualClass\selectlanguage{english}
} match first pattern and are thus excluded. Now, let's clear the exclusion
...
\var{svTest} objects, as well as tests attached to objects in the
...
Now let\textquoteright{}s make a test unit using tests collected in
this suite:
...
your suite is very simple. You still use \code{\inputencoding{latin1}runTest()\selectlanguage{english}
} as usual, but this time, you apply it to your \var{svSuite} object:
...
the \pkg{svUnit} package, including functions to define the content
...
\item Click the \menuitem{Run} button each time you want to refresh the
test tree,
...
Komodo.
\end{itemize}
The \menuitem{Auto} mode, when activated, sources \R{} files currently
...
to look at the \code{\inputencoding{latin1}koUnit\_XXX()\selectlanguage{english}
} functions in the \pkg{svUnit} package. These functions allow to
control the GUI in Komodo remotely from within \R{}, and similar
functions should not be too difficult to implement for other GUIs. \end{framed}
