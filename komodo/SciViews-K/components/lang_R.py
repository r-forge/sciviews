...
class RLangIntel(CitadelLangIntel, ParenStyleCalltipIntelMixin,
    ...
    #---- [PhG] Use statusbar for quick debugging
    def _debug_statusbar(msg):
        ...
        sm.category = "RCodeIntel.Debug"
        sm.msg = msg
        sm.timeout = 1000    # 0 for no timeout, else a number of milliseconds
    ...
    #
    def trg_from_pos(self, buf, pos, implicit=True, DEBUG=False, lang="R"):  #ac=None):
        DEBUG = True
        if pos < 1:
            return None
        ...
        if DEBUG:
            RLangIntel._debug_statusbar("trg_from_pos: char: %r, style: %d" % (char, accessor.style_at_pos(last_pos), ))
            print "trg_from_pos: char: %r, style: %d" % (char, accessor.style_at_pos(last_pos), )
        if char == '$' or char == '@' or char == '[':
            # Variable completion trigger.
            if DEBUG:
                RLangIntel._debug_statusbar("triggered:: complete variables")
    ...
    def preceding_trg_from_pos(self, buf, pos, curr_pos,
        ...
        #if style in (SCE_UDL_CSL_VARIABLE, ):
        #    start, end = accessor.contiguous_style_range_from_pos(last_pos)
        #    if DEBUG:
        #        print "triggered:: complete variables"
        #    return Trigger(self.lang, TRG_FORM_CPLN, "variables",
...
#            if token.get('style') == SCE_UDL_CSL_WORD:
#            #if token.get('style') == SCE_UDL_CSL_VARIABLE:
...
#    _identifier_cplns = [ ("function" "test1") ("variable" "test2") ("namespace" "ns") ("keyword" "key") ]
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=RLexer(mgr),
        buf_class=RBuffer,
        langintel_class=RLangIntel,
        # TODO: Should implement this: look at search() path and
        # import fct names. Look at Tcl for an example
